...
export function MembersClient({
  ...
  const filterGender = normalizeGender(genderParam ?? null);
  const filterLevel = normalizeLevel(levelParam ?? null);
  ...
  function updateFilters(updates: {
    q?: string;
    gender?: Gender | "all";
    level?: LevelGrade | "all";
  }) {
    const nextSearchParams = new URLSearchParams(currentQueryString);

    if (updates.q !== undefined) {
      if (updates.q.trim()) {
        nextSearchParams.set("q", updates.q);
      } else {
    ...
    if (updates.gender !== undefined) {
      if (updates.gender === "all") {
        ...
        nextSearchParams.set("gender", updates.gender);
      }
    }
    ...
    if (updates.level !== undefined) {
      if (updates.level === "all") {
        ...
        nextSearchParams.set("level", updates.level);
      }
    }
    ...
    <div className="p-4">
      ...
      <div className="flex gap-1.5 mb-3.5 overflow-x-auto pb-1 scrollbar-hide">
        <FilterChip
          ...
          onClick={() => updateFilters({ gender: "all", level: "all" })}
        />
        <FilterChip
          label="남성"
          active={filterGender === "male"}
          onClick={() =>
            updateFilters({ gender: filterGender === "male" ? "all" : "male" })
          }
        />
        <FilterChip
          label="여성"
          ...
          onClick={() =>
            updateFilters({ gender: filterGender === "female" ? "all" : "female" })
          }
        />
        {(["A", "B", "C", "D", "E"] as LevelGrade[]).map((grade) => (
          <FilterChip
            ...
            onClick={() =>
              updateFilters({ level: filterLevel === grade ? "all" : grade })
            }
          />
        ))}
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-[var(--color-border)]">
          {filteredMembers.map((member) => (
            <MemberListItem
              key={member.id}
              member={member}
              onClick={(selectedMember) => openMemberDetail(selectedMember.id)}
            />
          ))}
        </div>
      )}
...
function normalizeGender(value: string | null): Gender | "all" {
  return value === "male" || value === "female" ? value : "all";
}

function normalizeLevel(value: string | null): LevelGrade | "all" {
